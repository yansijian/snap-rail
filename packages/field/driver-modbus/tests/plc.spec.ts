...
import type { ModbusPointConfig } from '@snap-rail/protocol'
...
describe('decodePoint', () => {
  it('sign-extends i16 and reads u16 unsigned', () => {
    expect(decodePoint(point({ var: 'a', encoding: 'i16' }), { registers: [0xfffe] }, 0)).toBe(-2n)
    expect(decodePoint(point({ var: 'b', encoding: 'u16' }), { registers: [0xfffe] }, 0)).toBe(65534n)
  })

  it('decodes i32/u32 in both word orders', () => {
    const negative = point({ var: 'a', encoding: 'i32' })
    expect(decodePoint(negative, { registers: [0xffff, 0xfffe] }, 0)).toBe(-2n)
    const swapped = point({ var: 'b', encoding: 'i32', byteOrder: 'cdab' })
    expect(decodePoint(swapped, { registers: [0xfffe, 0xffff] }, 0)).toBe(-2n)
    const wide = point({ var: 'c', encoding: 'u32' })
    expect(decodePoint(wide, { registers: [1, 0] }, 0)).toBe(65536n)
  })

  it('decodes f32 with word order and applies scale', () => {
    const plain = point({ var: 'a', encoding: 'f32', type: 'float' })
    expect(decodePoint(plain, { registers: floatRegs(23.5) }, 0)).toBeCloseTo(23.5)
    const swapped = point({ var: 'b', encoding: 'f32', type: 'float', byteOrder: 'cdab' })
    expect(decodePoint(swapped, { registers: floatRegs(23.5, 'cdab') }, 0)).toBeCloseTo(23.5)
    const scaled = point({ var: 'c', encoding: 'f32', type: 'float', scale: 0.1 })
    expect(decodePoint(scaled, { registers: floatRegs(100) }, 0)).toBeCloseTo(10)
  })

  it('reads coils as booleans', () => {
    const coil = point({ var: 'a', encoding: 'coil', type: 'bool', fc: 1 })
    expect(decodePoint(coil, { bits: [false, true] }, 1)).toBe(true)
  })
})
...
describe('encodeWrite', () => {
  it('encodes coils from booleans only', () => {
    ...
    expect(encodeWrite(coil, true)).toEqual({ coil: true })
    expect(() => encodeWrite(coil, 1)).toThrow(/boolean/)
  })

  it('range-checks and encodes 16-bit registers', () => {
    const u16 = point({ var: 'a', encoding: 'u16', writable: true })
    expect(encodeWrite(u16, 65535n)).toEqual({ registers: [65535] })
    expect(() => encodeWrite(u16, 65536n)).toThrow(/range/)
    const i16 = point({ var: 'b', encoding: 'i16', writable: true })
    expect(encodeWrite(i16, -2n)).toEqual({ registers: [0xfffe] })
  })

  it('encodes 32-bit words in the configured order', () => {
    const big = point({ var: 'a', encoding: 'i32', writable: true })
    expect(encodeWrite(big, -2n)).toEqual({ registers: [0xffff, 0xfffe] })
    const swapped = point({ var: 'b', encoding: 'i32', byteOrder: 'cdab', writable: true })
    expect(encodeWrite(swapped, -2n)).toEqual({ registers: [0xfffe, 0xffff] })
  })

  it('reverses scale on f32 writes', () => {
    const scaled = point({ var: 'a', encoding: 'f32', type: 'float', scale: 0.1, writable: true })
    const encoded = encodeWrite(scaled, 10)
    const view = new DataView(new ArrayBuffer(4))
